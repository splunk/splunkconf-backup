...
admin password creation (Full, required existing or via user-seed.conf, UF no account creation required (unless you provide user-seed.conf file)
	...
	--splunk_subsys=|sub= name of Splunk service (splunk or splunkforwarder or the instance name) ?(default=splunk)
        ...
        --splunkrole=xxx run specific code for a specific role (currently implemented : ds,uf. Other values mean splunk enterprise) 
...
if ($RES) {
  ...
  print " version detection via splunk command failed , may be because extra message during a upgrade, trying fallback method via rpm version\n";
